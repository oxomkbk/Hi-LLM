with expected as (
  select 'skilltrustbench-t01-t09+priority-material+document-evidence-v6-scoped-credentials'::text as rules
), changed as (
  update public.ds_ai_security_settings settings
  set adapter_versions = jsonb_set(
        settings.adapter_versions,
        '{skill,rules}',
        to_jsonb(expected.rules),
        true
      ),
  ...
  where settings.id = true
    and settings.adapter_versions #>> '{skill,rules}' is distinct from expected.rules
...
set report_state = case when state.latest_assessment_id is null then 'unassessed' else 'stale' end,
    ...
    updated_at = now()
where state.subject_type in ('skill', 'skill_submission')
