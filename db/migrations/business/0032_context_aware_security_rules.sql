with expected as (
  select '{
    "skill": "skilltrustbench-t01-t09+priority-material+document-evidence-v2-context-aware",
    "mcp": "mcp-config+priority-source+document-evidence-v2-context-aware",
    "prompt": "prompt-safety-rules+document-evidence-v2-context-aware"
  }'::jsonb as rules
), changed as (
  ...
  set adapter_versions = jsonb_set(
        ...
        '{prompt,rules}', expected.rules -> 'prompt', true
      ),
  ...
  where settings.id = true
    and (
      ...
      or settings.adapter_versions #>> '{prompt,rules}' is distinct from expected.rules ->> 'prompt'
    )
