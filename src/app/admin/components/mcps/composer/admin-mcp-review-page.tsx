...
import { mcpComposerPayload, mcpToComposerValue } from '@/components/mcp-composer/types'
...
import type { McpSubmission } from '@/types'

export default function AdminMcpReviewPage({ returnHref = '/admin/mcp/submissions', submissionId }: { returnHref?: string, submissionId: string }) {
  const router = useRouter()
  const [submission, setSubmission] = useState<McpSubmission | null>(null)
  ...
  useEffect(() => {
    let active = true
    request<McpSubmission>(`/mcp-submissions/${submissionId}`)
      .then((result) => {
        if (active)
  ...
  const submit = async (value: McpComposerValue, intent: McpSubmitIntent, reviewNote?: string) => {
    if (!submission)
      return false
    setSubmitting(true)
    setError(null)
    try {
      const content = mcpComposerPayload(value)
      if (intent === 'save') {
        const result = await request<McpSubmission>(`/mcp-submissions/${submission.id}`, {
          body: JSON.stringify({ ...content, expected_updated_at: submission.updated_at }),
          method: 'PUT',
        })
        ...
        const refreshed = await request<McpSubmission>(`/mcp-submissions/${submission.id}`)
      ...
      const result = await request<{ id?: string, status: string }>(`/mcp-submissions/${submission.id}/review`, {
        body: JSON.stringify(intent === 'approve'
          ? { action: 'approve', content, expected_updated_at: submission.updated_at, review_note: reviewNote?.trim() || undefined }
          : { action: 'reject', expected_updated_at: submission.updated_at, review_note: reviewNote?.trim() }),
        method: 'POST',
      })
      if (result.code !== RESPONSE.SUCCESS)
        return false
  ...
  useSubmissionSecurityPolling<McpSubmission>({
    enabled: securityScanActive && Boolean(submission),
    endpoint: `/mcp-submissions/${submissionId}`,
    ...
    <McpComposer
      ...
      asideContent={(
        <SubmissionSecurityCard
          ...
          returnHref={buildContextualHref(`/admin/mcp/submissions/${submission.id}/review`, returnHref)}
          scanning={scanning}
      ...
      backHref={returnHref}
      baseUpdatedAt={submission.updated_at}
      draftKey={`hillm-nav:mcp-composer:v1:submission-review:current:${submission.id}`}
      draftStorage="session"
      error={error}
      initialValue={mcpToComposerValue(submission)}
